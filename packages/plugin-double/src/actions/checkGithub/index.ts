...
export class CheckGithubAction implements Action {
    ...
    async validate(
        ...
        if (!message.content.text && !message.content.url) {
            elizaLogger.error(
                "[❌ DOUBLE] CheckGithubAction - No text or URL provided"
            );
            return false;
        }

        const isExplicitCheck = message.content.action === "CHECK_GITHUB";
        const hasGithubKeywords = /github|repo|repository/i.test(
            message.content.text || ""
        );
        const hasGithubUrl = /github\.com/i.test(
        ...
        if (isValid) {
            elizaLogger.info(
                "[✅ DOUBLE] CheckGithubAction - Validation successful"
            );
        } else {
            elizaLogger.error(
                "[❌ DOUBLE] CheckGithubAction - Validation failed"
            );
        }
    ...
    async handler(
        ...
        try {
            elizaLogger.info(
                "[🔄 DOUBLE] CheckGithubAction - Processing GitHub check request"
            );
            ...
            elizaLogger.info(
                "[🔄 DOUBLE] CheckGithubAction - Analyzing repository:",
                githubUrl
            );
            ...
            if (callback) {
                elizaLogger.info(
                    "[✅ DOUBLE] CheckGithubAction - Analysis complete:",
                    analysis
                );
                callback({
            ...
            elizaLogger.error(
                "[❌ DOUBLE] CheckGithubAction - Handler error:",
                error
            );
            return false;
        }
